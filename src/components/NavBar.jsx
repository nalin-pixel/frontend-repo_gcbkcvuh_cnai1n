...
export default function NavBar() {
  ...
  const linkClasses = ({ isActive }) =>
    `px-3 py-2 rounded-md text-sm tracking-wide transition-colors gold-underline ${
      isActive ? 'text-white bg-white/5' : 'text-blue-100/80 hover:text-white hover:bg-white/5'
    }`

  return (
    <header className="sticky top-0 z-40 backdrop-blur-xl bg-gradient-to-b from-slate-950/80 to-slate-900/30 border-b border-white/10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex h-16 items-center justify-between">
          <Link to="/" className="flex items-center gap-3 group">
            <div className="h-9 w-9 rounded-full bg-gradient-to-tr from-yellow-500 to-amber-300 ring-2 ring-white/20 shadow-[0_0_30px_rgba(234,179,8,0.35)] slow-zoom" />
            <div className="leading-tight">
              <p className="gold-text font-semibold tracking-wide">Solaris Studio</p>
              <p className="text-[10px] uppercase tracking-[0.2em] text-blue-200/70">Editorial Demos</p>
            </div>
          </Link>
